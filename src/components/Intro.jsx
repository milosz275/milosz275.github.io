...
function Intro() {
  ...
  return (
    <div className={`flex flex-col items-center justify-center text-center pt-20 pb-12 transition-all duration-500 ${hasScrolled ? "min-h-0" : "min-h-screen"}`}>
        <h1 className="text-4xl md:text-5xl text-white mb-1 md:mb-3 font-bold select-none">
            Miłosz Maculewicz
        </h1>
        <p className="text-base text-white max-w-xl mb-6 font-normal">
          Software Engineer, System Admin
        </p>
        {/* Description and subtitle are always visible */}
        <p className="text-sm text-white max-w-xl mb-6 font-normal">
        ...
        <div className={`opacity-0 ${hasScrolled ? "opacity-100 translate-y-0" : "translate-y-10"} transition-opacity duration-700 ease-out`}>
            {/* Additional content that becomes visible on scroll */}
